...
public class Controller32 {
    ...
    @GetMapping("sub1")
    @ResponseBody // 리턴 값이 응답 데이터다
    public String sub1() {
        System.out.println("Controller32.sub1");

        // view 로 forward가 아니라 client 에게 응답하는 값
        return "클라이언트로 전달할 값";
    }

    // 두번째 버튼 클릭시 응답데이터 가 콘솔에 출력되도록.
    @GetMapping("sub2")
    @ResponseBody
    public String sub2() {
        System.out.println("Controller32.sub2");
        return "응답 데이터";
    }
    ...
    public Map<String, Object> sub4() {
        System.out.println("Controller32.sub4");
    ...
    public MyBean321 sub7() {
        ...
        myBean321.setId(912);
        myBean321.setName("차범금");
        myBean321.setFruits(List.of("apple", "lemon", "orange"));
        myBean321.setAddress(Map.of("city", "신촌", "country", "한ㄱㄱ"));

        // json
        // {"id" : 912, "name" : "차범근",
        // "fruits" : ["apple" , "lemon"],
        // "address" : {"city" : "신촌" , "country" : "한국"}}
        return myBean321;
    }
    ...
    public ResponseEntity<Map<String, Object>> sub20() {
        return ResponseEntity.badRequest().body(
                Map.of("city", "서울", "text", "헬로"));
    }

    // 연습 : react 21번째, 22번째 버튼 클릭시 아래
    @GetMapping("sub21")
    @ResponseBody
